...
export default function AskPage() {
  ...
  async function approve(turnIdx: number, proposal: Proposal) {
    ...
    const final = edited !== undefined ? { ...proposal, body: edited } : proposal;
    setBusy(true);
    try {
      ...
      setTurns((t) =>
        t.map((turn, i) =>
          i === turnIdx ? { ...turn, proposalDone: d.success ? "sent" : null, text: d.success ? turn.text : `${turn.text}\n\nSend failed: ${d.error}` } : turn
        )
      );
    } finally {
      setBusy(false);
                ...
                {turn.proposal && (
                  <div style={proposalCard}>
                    <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 0.5, color: "var(--accent)", fontWeight: 600, marginBottom: 8 }}>
                      {turn.proposal.kind === "reply" ? "Proposed reply" : "Proposed email"}
                      {turn.proposalDone === "sent" && <span style={{ color: "var(--green)", marginLeft: 8 }}>✓ Sent</span>}
